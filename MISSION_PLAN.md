...
The system must be:
- 100% Savant-branded (no Hermes/ILO pollution in core identity)
- Fully documented
...
1. **Branding Lead** (subagent)
   - Finish renaming all remaining Hermes/ILO references in source, docs, examples, tests
   - Update class names, file names, docstrings, __init__.py exports
   - Goal: Zero "hermes" or "ilo" in user-facing or core Savant identity
...
- When Grok (or any model) spawns sub-agents, each gets `~/.savant/swarms/<swarm_id>/<subagent_id>/pool/`
