...
Every agent and every sub-agent in a swarm participates in the pool through the **SavantHarness**. The pool starts completely empty and grows only with real, high-signal experience from successful work.
...
```python
from savant import SavantPool, get_default_pool, PoolQuery, SavantHarness
...
### The SavantHarness (Preferred for Worker Agents)
...
harness = SavantHarness(agent_id="my-agent-007")
