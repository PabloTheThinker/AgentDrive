...
4. **Experience, patterns, and skills compound automatically — and merge.** Successful MCP/CLI operations auto-absorb via `run_operation` (on by default): lightweight `experience_graph_record_reasoning` when you skip it, plus inherited skill distillation + DNA ingest on high-signal ops. When a session combines **experience traces, distilled skills, and codebase patterns**, AgentDrive runs **growth merge** (pattern recognition across memory + graph + codebase) and **fuses** born skills (`fused-*`, not copies of parents). Check `auto_learning.growth_merge` and `auto_learning.fused_skill` on results. For maximum compounding context, call `growth_merge_briefing`. You may still call `experience_graph_record_reasoning` for richer traces.
