...
**DNA = Memory + Patterns** (Assassin’s Creed / evolutionary style).
...
Because the sub-agent uses the **SavantHarness** (or `RichAgentAdapter`), the pull-adapt-record loop happens automatically:
...
3. Each sub-agent:
   ...
   - Begins work using SavantHarness + RichAgentAdapter (or equivalent for its model).
...
```python
import os
from savant import SavantHarness, get_swarm_pool_path
from savant.pool.pool import SavantPool
from savant.pool.settings import get_effective_pool_settings
...
> “When you spawn sub-agents for this mission, assign each a unique SAVANT_SUBAGENT_ID under swarm_id ‘payment-refactor-2026’, attach the SavantHarness with the corresponding scoped pool, and set isolation_level=subagent with selective upward proposals.”
