...
def synthesize_framework(observations: Iterable[Any],
                            *, framework_id: str,
                            version: str = "0.1.0",
                            display_name: str = "",
                            category: str = "custom") -> FrameworkSynthesis:
    ...
    if not by_kind:
        ...
        steps.append({"id": "summarize", "type": "reasoning",
                       "agent": "savant-core", "output": "summary"})
        ...
        for idx, (kind, group) in enumerate(sorted(by_kind.items())):
            ...
            inputs.append({
                ...
                "required": False,
                "description": (
                    f"Optional filter for the {kind!r} observations "
                    f"({len(group)} seen)."
                ),
            })
    ...
    output_schema = {
        ...
        "required": [
            ...
            "engagement_id", "produced_at", *schema_required,
        ],
        "properties": {
...
def synthesis_summary(synthesis: FrameworkSynthesis) -> str:
    ...
    counts = Counter(s["id"].split("_")[-1] for s in synthesis.steps
                       if s["id"].startswith("analyze_"))
    parts = [f"framework {synthesis.framework_id!r} v{synthesis.version}:"]
    parts.append(f"{len(synthesis.inputs)} inputs, "
