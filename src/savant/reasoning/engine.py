...
class ReasoningEngine:
    ...
    # ------------------------------------------------------------------ #
    def build_causal_graph(self, trace: ReasoningTrace, observations: list[dict] | None = None) -> CausalGraph:
        """Thin wrapper around mine_causality with ledger discipline."""
        with self.ledger.record("reasoning.build_causal_graph",
                                summary=f"causal analysis for {self.genome_id}"):
    ...
    def synthesize_framework_steps(
        ...
        **kwargs,
        ...
        with self.ledger.record("reasoning.synthesize_framework",
                                summary=f"synthesize steps for {fid}"):
            return synthesize_framework(
                observations, framework_id=fid, **kwargs
            )
