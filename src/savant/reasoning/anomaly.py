...
def detect_anomalies(observations: Iterable[Any],
                       *, prior_observations: Iterable[Any] | None = None,
                       rare_threshold: float = 0.05,
                       stale_threshold_seconds: float = 86_400.0) -> list[Anomaly]:
    ...
    # 3) Stale rule.
    timestamps = [float(_get(o, "observed_at") or 0.0) for o in rows
                    if _get(o, "observed_at")]
    if len(timestamps) >= 3:
        ...
        for o in rows:
            ...
            if ts and median_ts - ts > stale_threshold_seconds:
                anomalies.append(Anomaly(
                    rule="stale", kind=str(_get(o, "kind") or ""),
    ...
    if prior_observations is not None:
        prior_keys = {
            f"{_get(p, 'kind')}:{_get(p, 'identity')}"
            for p in prior_observations
        }
        for o in rows:
            ...
            if k and iid and f"{k}:{iid}" not in prior_keys:
                anomalies.append(Anomaly(
                    rule="identity-novel", kind=k, identity=iid,
