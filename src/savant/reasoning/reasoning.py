...
class ReasoningTrace:
    ...
    def render(self) -> str:
        if not self.steps:
            return "(empty trace)"
        lines = [f"reasoning trace — {len(self.steps)} step(s), "
...
def reconstruct_trace(entries: Iterable[dict[str, Any]]) -> ReasoningTrace:
    ...
    for idx, rid in enumerate(order, start=1):
        ...
        step = TraceStep(
            ...
            counts={k: int(v) for k, v in (e.get("counts") or {}).items()
...
def explain_trace(trace: ReasoningTrace) -> str:
    ...
    top_ops = ", ".join(
        f"{op}({n})" for op, n in
        sorted(trace.operation_counts.items(), key=lambda p: -p[1])[:5]
    )
    top_sources = ", ".join(
        f"{src}({n})" for src, n in
        sorted(trace.citations_summary.items(), key=lambda p: -p[1])[:5]
    ) or "(no citations recorded)"
    fail_summary = (
        f"; {len(trace.failed)} failure(s): "
        + ", ".join(f.operation for f in trace.failed[:3])
        if trace.failed else ""
    )
    return (
