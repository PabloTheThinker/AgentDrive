...
- Stored under `~/.savant/reasoning/patterns/<corpus>.jsonl`
...
def _savant_reasoning_root() -> Path:
    ...
    if base:
        return Path(base)
    return Path.home() / ".savant" / "reasoning"
...
class PatternSignature:
    ...
    def to_dict(self) -> dict:
        return {"framework_id": self.framework_id,
    ...
    @classmethod
    def from_run_data(cls, framework_id: str, intents: list[str],
                       fields: list[str], display_name: str = "") -> "PatternSignature":
...
class PatternMemory:
    ...
    def recognize(self, signature: PatternSignature,
                   *, k: int = 3,
                   min_score: float = 0.1) -> list[PatternMatch]:
        ...
        for s in sigs:
            ...
            score = 0.5 * io + 0.5 * fo
            if score >= min_score:
                matches.append(PatternMatch(signature=s, score=score,
                                              intent_overlap=io, field_overlap=fo))
...
def summarize_matches(matches: Iterable[PatternMatch]) -> str:
    ...
    fragments = [
        f"{m.signature.framework_id}@{m.score:.2f} "
        f"(seen×{m.signature.occurrences})"
        for m in rows
    ]
