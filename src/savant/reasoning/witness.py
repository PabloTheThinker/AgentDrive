...
_VAGUE_QUANTIFIERS = (
    "some", "many", "a few", "several", "lots of", "a lot of",
    "most", "various", "numerous", "plenty of", "a number of",
)
_VAGUE_TIMES = (
    "recently", "earlier", "a while ago", "lately", "soon",
    "sometime", "before", "after a bit", "in the past",
)
...
class Citation:
    ...
    def render(self) -> str:
        ...
        return f"[{self.source}:{self.source_id} @ {int(self.observed_at)}" + \
            (f" {body}" if body else "") + "]"
...
class NumericClaim:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "statement": self.statement, "count": self.count,
...
def witness_claim(statement: str, *, count: int,
                    source: str, source_id: str,
                    fields: dict[str, Any] | None = None,
                    confidence: float = 1.0) -> NumericClaim:
    """Build a NumericClaim with a single citation."""
    return NumericClaim(
        statement=statement, count=int(count), confidence=float(confidence),
        citations=[Citation(source=source, source_id=source_id,
                              fields=fields or {})],
    )
