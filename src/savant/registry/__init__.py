...
from savant.genome.models import Genome, GenomeAuthor
...
class GenomeRegistry:
    ...
    def __init__(self, root: Path | None = None, *, swarm_id: Optional[str] = None, subagent_id: Optional[str] = None):
        self.swarm_id = swarm_id
        self.subagent_id = subagent_id
        if root is None:
            from savant.constants import get_savant_home, get_swarm_pool_path
            if swarm_id is not None or subagent_id is not None:
                pool_p = get_swarm_pool_path(swarm_id or "default", subagent_id)
                root = Path(pool_p) / "genomes"
            else:
                root = get_savant_home() / "genomes"
        self.root = Path(root)
    ...
    def load(self, genome_id: str) -> Optional[Genome]:
    ...
    def get_genome_path(self, genome_id: str) -> Optional[Path]:
        path = self.root / genome_id
        ...
        for name in self.list_genomes():
            ...
            try:
                ...
                details.append({
                    ...
                    "id": m.id,
                    "version": m.version,
                    "domains": m.applicability.get("domains", []) if isinstance(m.applicability, dict) else [],
                    "problem_signatures": m.applicability.get("problem_signatures", []) if isinstance(m.applicability, dict) else [],
                    "score": float(m.evaluation_score.get("reference_tasks", 0.0)) if isinstance(m.evaluation_score, dict) else 0.0,
                    ...
                    "last_improved": (m.last_improved.isoformat() if m.last_improved and hasattr(m.last_improved, "isoformat") else None),
                    "has_framework": bool(g.framework),
                    "num_steps": len(g.framework.get("steps", [])) if g.framework else 0,
                })
            except Exception:
                details.append({
                    "dir_name": name, "genome_id": name, "id": name, "version": "?", "domains": [],
                    "problem_signatures": [], "score": 0.0, "authors": ["?"], "created": "",
                    "last_improved": None, "has_framework": False, "num_steps": 0,
                })
        return sorted(details, key=lambda d: d.get("genome_id", d["dir_name"]))

    def get_genome(self, key: str) -> Optional[Genome]:
    ...
    def search_genomes(self, query: str) -> List[str]:
        ...
        for d in self.list_genome_details():
            haystack = " ".join(filter(None, [
                d.get("id", ""), d.get("genome_id", ""), d.get("dir_name", ""),
                " ".join(d.get("domains", [])), " ".join(d.get("problem_signatures", []))
            ])).lower()
            if q in haystack:
                hits.append(d["dir_name"])
        return hits

    def ensure_bootstrap_example(self, example_dir: Optional[Path] = None) -> Optional[str]:
        """Auto-register the seed example genome if registry is empty. Returns registered genome_id or None."""
        if self.list_genomes():
            return None
        if example_dir is None:
            candidates = [
                Path("/home/pablothethinker/savant/genomes/examples/security-incident-postmortem-v1"),
                Path.cwd() / "genomes" / "examples" / "security-incident-postmortem-v1",
                Path(__file__).resolve().parents[4] / "genomes" / "examples" / "security-incident-postmortem-v1",  # src/savant/registry/__init__.py -> project
            ]
            for c in candidates:
    ...
    def get_registry_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_score": round(sum(det.get("score", 0) for det in details) / max(1, len(details)), 3) if details else 0.0,
    ...
    def load(self, genome_id: str) -> Optional[Genome]:  # type: ignore[override]
        ...
        gid, ver = self._parse_spec(genome_id) if hasattr(self, "_parse_spec") else (genome_id, None)
        ...
        for cand in candidates:
            p = self.root / cand
            if p.exists() and p.is_dir() and ((p / "manifest.json").exists() or (p / "manifest.yaml").exists()):
                try:
                    return Genome.load(p)
                except Exception:
                    continue
    ...
    def _parse_spec(self, spec: str) -> tuple[str, str | None]:
        ...
        if "-" in s:
            ...
            if right and (right[0].isdigit() or right[0].isalpha() and any(c.isdigit() for c in right)):
                return left, right
            # fallback whole
        return s, None
    ...
    def search(
        ...
        for name in self.list_genomes():
            ...
            if query:
                q = query.lower()
                hay = " ".join([
                    g.manifest.id, g.manifest.version,
    ...
    def fork(
        ...
        if src is None:
            raise ValueError(f"Fork source '{source_spec}' not found. Registry has: {self.list_genomes()[:10]}")
        child = src.fork(new_version=new_version, authors=authors, notes=notes)
        self.save(child)
        return child

    def register_example_if_needed(self) -> Optional[str]:
