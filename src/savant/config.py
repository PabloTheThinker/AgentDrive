...
- ~/.savant/config.yaml for user settings
- ~/.savant/.env for secrets (API keys etc.)
- Strong defaults
- Caching, warnings on parse failure
- Context-aware SAVANT_HOME
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "integration": {
        ...
        # legacy key retained for transition compatibility
        "hermes": {
            "enabled": False,
...
def _normalize_config(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for section in ("savant", "registry", "scanners", "orchestrator", "tui", "integration", "pool"):
...
def read_raw_config() -> Dict[str, Any]:
    ...
    with _CONFIG_LOCK:
        ...
        try:
            ...
            _RAW_CONFIG_CACHE[key] = (st.st_mtime_ns, st.st_size, raw.copy() if isinstance(raw, dict) else {})
...
def setup_logging(level: str | None = None, log_file: Path | None = None) -> None:
    ...
    cfg = load_config()
    lvl = level or cfg.get("savant", {}).get("log_level", "INFO")
    numeric = getattr(logging, lvl.upper(), logging.INFO)

    log_dir = get_savant_home() / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    if log_file is None:
        log_file = log_dir / "savant.log"
    ...
    logger.info(f"Savant v{SAVANT_VERSION} logging initialized (level={lvl}, file={log_file})")
...
def get_logger(name: str | None = None) -> logging.Logger:
    ...
    if not _LOGGING_CONFIGURED:
        setup_logging()
    return logging.getLogger(name or "savant")
