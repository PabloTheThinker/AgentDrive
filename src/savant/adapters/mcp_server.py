"""
Savant MCP Server — Universal bridge exposing Savant Pool operations to any MCP-capable AI.

This server lets Claude Code, Cursor, Codex, Grok clients, Windsurf, etc. talk to the
user's Savant Pools using the standard Model Context Protocol (MCP) without any
...
Any model the user tells "use your Savant Pool for this swarm" can be pointed at
...
The user remains fully in control via ~/.savant/config.yaml (PoolSettings).
"""
...
logger = logging.getLogger("savant.mcp_server")
...
def _get_adapter():
    ...
    from savant.adapters.base import SavantAdapterBase, get_scoped_pool

    return SavantAdapterBase(name="mcp")


def _get_pool(swarm_id: Optional[str] = None, subagent_id: Optional[str] = None):
    from savant.adapters.base import create_scoped_pool
...
def _format_genome_brief(g: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "id": getattr(m, "id", None) if hasattr(m, "id") else (m.get("id") if isinstance(m, dict) else None),
            "version": getattr(m, "version", None) if hasattr(m, "version") else (m.get("version") if isinstance(m, dict) else None),
            "score": (m.evaluation_score or {}).get("reference_tasks", 0.0) if hasattr(m, "evaluation_score") else 0.0,
        }
    except Exception:
...
def create_mcp_server() -> "FastMCP":
    """Build the FastMCP instance and register all Savant Pool tools."""
    ...
    mcp = FastMCP(
        "savant",
        instructions=(
            "Savant Pool MCP Server — the living DNA / genome repository for AI agents.\n"
    ...
    def savant_pool_status() -> str:
        """Current status of the active (possibly scoped) Savant Pool."""
    ...
    def savant_pool_query(
        ...
        """Semantic search across the Savant Pool for relevant Genomes/DNA.

        Returns enriched packets with relevance explanations (why each genome matches).
        """
        pool = adapter.get_scoped_pool()
        from savant.pool.pool import PoolQuery
    ...
    def savant_list_swarms() -> str:
        ...
        from savant.constants import get_swarms_dir
    ...
    @mcp.tool()
    def savant_get_swarm_pool(swarm_id: str, subagent_id: Optional[str] = None) -> str:
    ...
    @mcp.tool()
    def savant_get_settings(swarm_id: Optional[str] = None) -> str:
        """User-controlled PoolSettings (isolation, auto-ingest, sharing policy)."""
    ...
    def savant_record_outcome(
        ...
        """Record a completed task outcome so the pool can learn / evolve.
        ...
        from savant.adapters.base import create_harness
    ...
    def savant_ingest_genome(
        ...
        try:
            ...
            return json.dumps({"accepted": res.accepted, "genome_id": res.genome_id, "reason": res.reason})
        except Exception as exc:
            return json.dumps({"error": str(exc), "hint": "Use full Python Savant API for complex genomes."})
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Savant MCP Server exposing Pool DNA to any MCP client")
    parser.add_argument(
        "--transport",
