"""
Savant Adapters — Multi-Model Integration for Savant Pools

This package enables *any* AI model or agent system (Grok Build, Claude Code,
Codex, Cursor, custom agents, etc.) to participate in the Savant ecosystem of
shared, evolving agent DNA (Genomes) stored in user-owned Pools — including
automatic scoping for swarms and sub-agents.

Core value:
- User tells their model: "use your Savant Pool for this swarm"
- The model activates the appropriate adapter.
- All work (and sub-agent work) pulls relevant DNA, contributes improvements back.
- Sub-agents automatically receive isolated, persistent, user-controlled Savant Pools
  via environment variables (SAVANT_SWARM_ID, SAVANT_SUBAGENT_ID) or explicit context.

Universal access:
- Direct Python API (any model that can exec Python or has tool access)
- MCP server (stdio for Claude Desktop / Cursor / Codex MCP clients, or HTTP)

See individual adapter modules for model-specific integration points and
ready-to-paste instruction text for the user to give their AI.

Public API (recommended entry points):
    from savant.adapters import (
        get_savant_adapter,
        SavantAdapter,
        activate_for_grok_build,
        get_scoped_pool,
    )

    # Universal
    pool = get_scoped_pool()  # auto-detects swarm/sub from env or defaults

    # Model-specific activation
    adapter = activate_for_grok_build(swarm_id="my-research-swarm")
"""

from __future__ import annotations

from typing import Optional, TYPE_CHECKING

from .base import (
    SavantAdapter,
    SavantAdapterBase,
    get_scoped_pool,
    get_savant_pool,
    detect_swarm_context,
    create_scoped_pool,
)

# Re-export for convenience
from savant.pool.pool import SavantPool
from savant.pool.settings import PoolSettings, get_effective_pool_settings

# Model-specific adapters (import lazily to avoid heavy deps at top level)
def get_savant_adapter(model: str = "auto") -> "SavantAdapter":
    """Factory: return the best adapter for the given model family.

    model: "grok" | "grok-build" | "claude" | "claude-code" | "codex" | "auto"
    """
    model = (model or "auto").lower()
    if model in ("grok", "grok-build", "grok_build"):
        from .grok_build_adapter import GrokBuildSavantAdapter

        return GrokBuildSavantAdapter()
    if model in ("claude", "claude-code", "claude_code"):
        from .claude_code_adapter import ClaudeCodeSavantAdapter

        return ClaudeCodeSavantAdapter()
    if model in ("codex", "openai-codex"):
        from .codex_adapter import CodexSavantAdapter

        return CodexSavantAdapter()
    # default / auto
    from .base import SavantAdapterBase

    return SavantAdapterBase()


def activate_for_grok_build(swarm_id: Optional[str] = None, **kwargs) -> "SavantAdapter":
    """One-liner for Grok Build users: call this after user says 'use Savant Pool'.

    Returns the activated adapter. Automatically patches spawn_subagent if possible.
    """
    from .grok_build_adapter import GrokBuildSavantAdapter, get_savant_instructions_for_grok

    adapter = GrokBuildSavantAdapter(swarm_id=swarm_id, **kwargs)
    adapter.activate()
    # Print user-friendly instructions the model can surface if needed
    # (models often echo this back to confirm)
    print(get_savant_instructions_for_grok(swarm_id or "current-session"))
    return adapter


def activate_for_claude(swarm_id: Optional[str] = None, **kwargs) -> "SavantAdapter":
    from .claude_code_adapter import ClaudeCodeSavantAdapter

    adapter = ClaudeCodeSavantAdapter(swarm_id=swarm_id, **kwargs)
    adapter.activate()
    return adapter


__all__ = [
    "SavantAdapter",
    "SavantAdapterBase",
    "get_savant_adapter",
    "get_scoped_pool",
    "get_savant_pool",
    "detect_swarm_context",
    "create_scoped_pool",
    "activate_for_grok_build",
    "activate_for_claude",
    "SavantPool",
    "PoolSettings",
    "get_effective_pool_settings",
]

# Lazy submodules for direct access: from savant.adapters.grok_build_adapter import ...
# (no __getattr__ needed for normal usage)
