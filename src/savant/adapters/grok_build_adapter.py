...
1. The user can say to Grok: "use your Savant Pool for this swarm"
2. Grok activates the adapter (once).
3. Every time Grok (or its sub-agents) calls the native `spawn_subagent(...)`,
   the children are automatically given their own isolated Savant Pool via
   environment variables + optional context injection.
4. Sub-agents, when they start, simply call `from savant.adapters import get_scoped_pool`
...
spawn, participate in the Savant Pool ecosystem. At the very beginning of your
thinking or code, execute:

    from savant.adapters.grok_build_adapter import (
...
SAVANT_SWARM_ID + SAVANT_SUBAGENT_ID and therefore its own persistent DNA pool.
...
from savant.adapters.base import SavantAdapterBase, SavantContext, create_scoped_pool, detect_swarm_context
...
def get_savant_instructions_for_grok(swarm_id: str = "current-mission") -> str:
    ...
    to activate Savant Pool participation for a swarm.
    """
    return f"""
Use the Savant Pool system for all work in this swarm (id: {swarm_id}).
...
children will automatically receive isolated Savant Pools (SAVANT_SWARM_ID +
SAVANT_SUBAGENT_ID are injected). Inside every agent (parent and child) simply
do:
    ...
    harness = SavantHarness(agent_id=..., pool=pool)

All DNA pulled and all high-quality outcomes recorded will live in the
user-owned, persistent, per-subagent pools under ~/.savant/swarms/{swarm_id}/...
...
class GrokBuildSavantAdapter(SavantAdapterBase):
    ...
    def __init__(self, swarm_id: Optional[str] = None, **kwargs: Any):
        ...
        self._original_spawn: Optional[Callable] = None
        self._patched = False
    ...
    def activate(self, swarm_id: Optional[str] = None, **options: Any) -> None:
        ...
        if self._swarm_id:
            os.environ.setdefault("SAVANT_SWARM_ID", self._swarm_id)
    ...
    def _find_spawn_subagent(self) -> Optional[Callable]:
    ...
    def _patch_spawn_subagent(self) -> bool:
        ...
        def savant_wrapped_spawn(*args: Any, **kwargs: Any) -> Any:
            ...
            swarm_id = (
                ...
                or current_swarm
                or self._swarm_id
                or os.environ.get("SAVANT_SWARM_ID", "grok-session")
            )
            ...
            ctx = SavantContext(
                ...
                parent_agent_id=os.environ.get("SAVANT_SUBAGENT_ID") or "grok-parent",
            )
    ...
    def spawn_subagent_with_savant(
        ...
        **kwargs: Any,
        ...
        swarm = swarm_id or self._swarm_id or os.environ.get("SAVANT_SWARM_ID", "grok-fallback")
        sub = subagent_id or f"sub-{id(args) % 100000:05d}"
        os.environ["SAVANT_SWARM_ID"] = swarm
        os.environ["SAVANT_SUBAGENT_ID"] = sub
...
spawn_subagent: Optional[Callable] = None  # will be set by first activation if patching works
