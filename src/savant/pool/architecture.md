...
### 1. SwarmPoolPolicy (the "blocked tools" equivalent)
...
### 2. SavantSwarmPoolManager (the active subagents registry)
Located in `swarm_manager.py`.

- Thread-safe registry of all active `(swarm_id, subagent_id)` → their private `SavantPool`
- Automatic provisioning of isolated directories under `~/.savant/swarms/<swarm_id>/<subagent_id>/pool/`
...
### 3. SavantPool + Harness Integration
- `SavantHarness` now understands swarm context (via constants or explicit `swarm_id`/`subagent_id`)
...
`settings.py` + `PoolSettingsManager`

Every policy is overridable by the owner:
- Via `~/.savant/config.yaml`
- Via CLI/TUI
...
child_pool = get_swarm_pool_manager().get_or_create_pool(
    swarm_id=swarm_id,
...
The child starts with an **empty** pool and grows its own unique DNA. The parent can see high-level summaries or selected DNA according to the active `SwarmPoolPolicy`.
