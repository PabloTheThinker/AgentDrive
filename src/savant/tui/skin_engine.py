...
class SkinEngine:
    ...
    def _load_skin(self, name: str) -> Dict[str, Any]:
        ...
        # Future: load from ~/.savant/skins/ or built-in presets
        if name == "default":
            return DEFAULT_SKIN
    ...
    def _build_rich_theme(self) -> Theme:
        c = self.skin["colors"]
        styles = {
            "savant.banner.border": f"bold {c['banner_border']}",
            "savant.banner.title": f"bold {c['banner_title']}",
            "savant.accent": c["ui_accent"],
            "savant.label": c["ui_label"],
            "savant.ok": c["ui_ok"],
            "savant.error": c["ui_error"],
            "savant.warn": c["ui_warn"],
            "savant.genome": c.get("genome_id", c["ui_accent"]),
            "savant.framework": c.get("framework_name", c["ui_label"]),
            "savant.evolution": c.get("evolution_step", "#AF7AC5"),
        }
        return Theme(styles)
