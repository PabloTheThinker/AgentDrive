...
from agentdrive.genome.models import Genome, GenomeAuthor, GenomeManifest
...
class AgentDriveRunScanner(BaseScanner):
    ...
    def scan(self, run_data: dict[str, Any] | Path | str) -> list[Genome]:
        ...
        manifest = GenomeManifest(
            ...
            authors=[
                GenomeAuthor(
                    type="agent", name=self.actor, run=str(data.get("timestamp", "unknown"))
                )
            ],
            applicability={
                ...
                "source_agent": "agentdrive",
            },
            dependencies={
                "genomes": [],
                "agent_capabilities": ["trajectory-parsing", "structural-reasoning"],
            },
