...
class CausalGraph:
    ...
    def render(self) -> str:
        ...
        lines = [f"causal graph — {self.node_count} step(s), {len(self.edges)} edge(s)"]
        for e in sorted(self.edges, key=lambda x: (x.src_seq, x.dst_seq)):
            lines.append(
                f"  {e.src_seq:>2} → {e.dst_seq:<2}  w={e.weight:.2f}  {e.rationale[:100]}"
            )
        return "\n".join(lines)

    def to_dict(self) -> dict:
        return {"node_count": self.node_count, "edges": [e.to_dict() for e in self.edges]}


def _tokens(text: str) -> set[str]:
    return {
        t.lower() for t in _TOKEN_RE.findall(text or "") if t.lower() not in _STOP and len(t) > 2
    }
...
def mine_causality(
    trace: ReasoningTrace, observations: Iterable[dict] | None = None
) -> CausalGraph:
    ...
    for i, dst in enumerate(steps):
        ...
        for src in steps[:i]:
            ...
            rare_shared = [t for t in shared_tokens if background.get(t, 0) <= 3]
            if rare_shared:
                weight += 0.1 * len(rare_shared)
                reasons.append(f"rare tokens carried: {sorted(rare_shared)[:5]}")
...
def explain_causality(graph: CausalGraph) -> str:
    ...
    for dst, incoming in sorted(by_dst.items()):
        ...
        top = incoming[0]
        fragments.append(
            f"step {dst} ← step {top.src_seq} (w={top.weight:.2f}, {top.rationale[:60]})"
        )
    return f"{len(graph.edges)} edge(s) across {graph.node_count} step(s). " + "; ".join(
        fragments[:5]
    )
