...
- Default storage: `~/.agentdrive/reasoning/ledger/YYYYMMDD.jsonl`
...
def ledger_path(root: Path | None = None, day: str | None = None) -> Path:
...
class Ledger:
    ...
    def tail(self, *, limit: int = 50, day: str | None = None) -> list[dict[str, Any]]:
...
def audited(operation: str, *, actor: str = "agentdrive", root: Path | None = None) -> Callable:
    ...
    def decorator(fn: Callable) -> Callable:
        ...
        if is_coro:
            ...
            async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
                with ledger.record(
                    operation=operation, metadata={"args": _short(args), "kwargs": _short(kwargs)}
                ) as entry:
...
def _absorb_counts(result: Any, entry: LedgerEntry) -> None:
    if isinstance(result, dict) and isinstance(result.get("counts"), dict):
        entry.counts.update({k: int(v) for k, v in result["counts"].items() if isinstance(v, int)})
